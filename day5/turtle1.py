...
while True:
    ...
    # 먹이 가장자리 이탈 확인
    if goal.xcor() > 290 or player.xcor() < -290:
        goal.right(180)

    if goal.ycor() > 290 or player.ycor() < -290:
        goal.right(180)


	# 플레이어 먹이 충돌 확인
    if isCollision(player, goal):
        goal.setposition(random.randint(-300, 200), random.randint(-300, 300))
